-- 140 — Pembukuan: tampilin ATRIBUSI (campaign) tiap order.
-- ============================================================================
-- Barry: di /orders/pembukuan pengen tau tiap order/lead itu dari campaign mana.
-- Atribusi = orders.campaign_id / orders_draft.campaign_id (hasil resolve parser
-- atribusi brief #14/#29) → tabel campaigns (campaign_name + platform kanonik).
--
-- Tambah 2 kolom output: campaign_name, campaign_platform. Sisanya IDENTIK dengan
-- migration 120 (NET presentation). laba_rugi_summary TIDAK diubah. Idempotent.
...
RETURNS TABLE(
  ...
  campaign_name TEXT, campaign_platform TEXT,
  total_count BIGINT
)
...
BEGIN
  ...
  WITH unioned AS (
    SELECT
      ...
      (SELECT campaign_name FROM public.campaigns WHERE id = d.campaign_id) AS campaign_name,
      (SELECT platform FROM public.campaigns WHERE id = d.campaign_id) AS campaign_platform
    ...
    SELECT
      ...
      (SELECT campaign_name FROM public.campaigns WHERE id = o.campaign_id) AS campaign_name,
      (SELECT platform FROM public.campaigns WHERE id = o.campaign_id) AS campaign_platform
  ...
  SELECT f.source, f.id, f.order_number, f.order_date, f.status, f.zone,
         ...
         f.campaign_name, f.campaign_platform,
