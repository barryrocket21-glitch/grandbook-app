-- 113 — list_pembukuan: HPP harus include packing_fee_snapshot (samain cost engine).
-- ============================================================================
-- Cost engine (compute_*_costs) ngitung HPP = qty×(hpp_snapshot + packing_fee_snapshot).
-- list_pembukuan (mig 111) masih qty×hpp_snapshot doang → selisih packing nyangkut
-- di est_fee_cs (derived = omset−hpp−gp), bikin Fee CS kegedean (938rb vs 592rb).
-- Fix: 2 subquery HPP (draft + terminal) tambah packing → est_hpp & est_fee_cs bener,
-- rekonsil 1:1 sama laba_rugi_summary. Cuma ganti 2 baris HPP, sisanya identik mig 111.
-- Idempotent.
