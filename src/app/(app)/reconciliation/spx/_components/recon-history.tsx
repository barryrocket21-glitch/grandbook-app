...
export function ReconHistory({ onResume }: { onResume?: (batchId: number) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    if (error) {
      ...
      setBatches((data || []) as unknown as ReconciliationBatch[])
    }
    setLoading(false)
  }, [])
      ...
      <CardContent className="pt-4 pb-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">History (10 terbaru)</h3>
          </div>
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-muted-foreground text-xs">
            Belum ada history reconciliation.
          </div>
        ) : (
