-- 100 — Brief #13: SATU TAMPILAN PEMBUKUAN (master view).
-- ============================================================================
-- Requirement Barry: SATU tabel pembukuan — semua order keliatan dalam satu
-- tampilan apapun statusnya (Nunggu Resi/Dikirim/Delivered/Retur/Batal),
-- status = kolom sortable/filterable. Pemisahan orders_draft (jalan) vs orders
-- (terminal) tetap di belakang layar, tapi view ini GABUNGIN (UNION) keduanya.
-- Gak ada order yang ilang dari pandangan setelah promote — cuma pindah status.
...
BEGIN
  ...
  RETURN QUERY
  WITH unioned AS (
    -- IN-FLIGHT (orders_draft): Baru / Antrian / Nunggu Resi / Dikirim / Problem
    SELECT
    ...
    UNION ALL
    -- TERMINAL (orders): DITERIMA→Arsip / RETUR→Retur / CANCEL→Batal / FAKE→Fake
    SELECT
